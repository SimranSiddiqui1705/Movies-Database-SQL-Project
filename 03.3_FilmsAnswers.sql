...
# Q1) Which categories of movies released in 2018? 
# Fetch with the number of movies. 

SELECT category.name, category.category_id,film.release_year, COUNT(category.category_id) AS number_of_films 
FROM category 
...
select category.name as category_name, count(film.film_id) as 'No of movies'
from film 
join film_category
on film.film_id = film_category.film_id
join category
...
# Q3) Add the new actors (id : 105 , 95) in film  
# ARSENIC INDEPENDENCE (id:41).
...
SELECT language.name, COUNT(language.language_id) AS number_of_films 
FROM language 
LEFT JOIN film 
...
select * from language where language_id = 2;  # Initially ITALIAN
...
ORDER BY COUNT(film.release_year) DESC 
LIMIT 1;
...
FROM film 
GROUP BY (film.release_year) 
ORDER BY COUNT(film.release_year) ASC 
LIMIT 1;
...
WHERE film_id = (SELECT film_id 
				FROM film 
                WHERE film.title = "GRAIL FRANKENSTEIN") AND 
...
VALUES ((SELECT category_id 
        ...
        (SELECT film_id 
        FROM film 
        WHERE film.title ="HARPER DYING")),
		((SELECT category_id 
        FROM category 
        WHERE category.name="Comedy"),
		(SELECT film_id 
        FROM film 
        WHERE film.title ="HARPER DYING")) as new
    ON DUPLICATE KEY UPDATE film_id = new.film_id , category_id = new.category_id;
...
# HINT: Delete all and and insert the new data to ensure no previous data is present.
DELETE FROM `film_actor` 
WHERE film_id = (SELECT film_id 
				FROM film 
...
VALUES ((SELECT actor_id 
		...
		((SELECT actor_id 
        FROM actor 
        WHERE actor.first_name = "MAE" AND actor.last_name="HOFFMAN"), 
        (SELECT film_id 
        FROM film 
        WHERE film.title="WEST LION")),
		((SELECT actor_id 
        FROM actor 
        WHERE actor.first_name = "SCARLETT" AND actor.last_name="DAMON"), 
        (SELECT film_id 
        FROM film 
        WHERE film.title="WEST LION"));
